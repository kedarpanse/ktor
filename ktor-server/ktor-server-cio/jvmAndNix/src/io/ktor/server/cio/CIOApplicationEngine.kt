...
public class CIOApplicationEngine(
    ...
    private fun initServerJob() {
        val coroutineScope = CoroutineScope(environment.parentCoroutineContext + engineDispatcher)
        serverJob = coroutineScope.launch(start = CoroutineStart.LAZY) {
            val connectors = ArrayList<HttpServer>(environment.connectors.size)
